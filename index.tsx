...
const VAPID_PUBLIC_KEY = 'BOO9O5KqIHXdB3z4RVeK4ZVVFdsUN7qN1TU8NpW2egebWLdH1l0bT9k_YxtCAJYgJolnxKQt6cMMC1db4zaXWEc';
...
async function handleOrderFormSubmit(e: Event) {
    ...
    try {
        ...
        if (imageFile) {
            // If editing and there's an old image, delete it first using the robust parser.
            if (isEditing && existingOrder?.image_url) {
                const oldPath = storagePathFromUrl(existingOrder.image_url, BUCKET_NAME);
                if (oldPath) {
            ...
            // Convert the image to WebP before uploading.
            console.log(`Original file: ${imageFile.name}, size: ${Math.round(imageFile.size / 1024)} KB`);
            const webpBlob = await convertImageToWebP(imageFile, { quality: 0.8 });
            ...
            // Standardize file path with user ID.
            const filePath = `${user.id}/${Date.now()}-${webpFile.name}`;
        ...
        if (isEditing) {
            // Chain .select() to get the updated row back from the database.
            ...
            orderData.is_urgent = false; // Default value for new orders
        ...
        // Text content is reset by the functions that open the modal.
...
function initEventListeners() {
  ...
  // 2. Touch-based Drag and Drop (for Mobile/Tablets)
...
/**
 * Handles the start of a touch event on the board.
 * Differentiates between long-pressing an image to copy, long-pressing the card
 * to toggle urgency (if authenticated), and initiating a drag action.
 */
function handleTouchStart(e: TouchEvent) {
    const target = e.target as HTMLElement;
    const card = target.closest('.order-card') as HTMLElement;
    if (!card) return;
    
    const isAuthenticated = document.body.classList.contains('role-authenticated');

    // Dragging is only for authenticated users.
    // Set up initial state for a potential drag.
    if (isAuthenticated) {
        state.touchDragState.initialX = e.touches[0].clientX;
        state.touchDragState.initialY = e.touches[0].clientY;
        state.touchDragState.draggedCard = card;
    }

    const isImagePress = target.tagName === 'IMG';

    // Start a timer for a long press action.
    state.longPressTimer = window.setTimeout(() => {
        const orderId = card.dataset.id;
        if (orderId) {
            const order = state.orders.find(o => o.id == orderId);
            if (order) {
                // Priority 1: If the long press is on an image with a URL, open it in a new tab.
                if (isImagePress && order.image_url) {
                    openImageInNewTab(order.image_url);
                    if (navigator.vibrate) navigator.vibrate(50);
                } 
                // Priority 2: If authenticated and not on an image, toggle urgency.
        ...
        state.touchDragState.draggedCard = null; // A long-press action prevents dragging.
    }, 500); // 500ms for long press
}

/**
 * Handles the movement of a touch event.
 * If movement is significant, cancels the long-press and starts dragging the card.
 */
function handleTouchMove(e: TouchEvent) {
    if (!state.touchDragState.draggedCard) return;

    const currentX = e.touches[0].clientX;
    const currentY = e.touches[0].clientY;
    const dx = Math.abs(currentX - state.touchDragState.initialX);
    const dy = Math.abs(currentY - state.touchDragState.initialY);

    if (dx > 5 || dy > 5) {
        // Cancel long press if it's still pending
        ...
        if (!state.touchDragState.isDragging) {
            state.touchDragState.isDragging = true;
            const card = state.touchDragState.draggedCard!;
            const rect = card.getBoundingClientRect();
            state.touchDragState.offsetX = state.touchDragState.initialX - rect.left;
            state.touchDragState.offsetY = state.touchDragState.initialY - rect.top;
        ...
        const card = state.touchDragState.draggedCard!;
        state.touchDragState.currentX = currentX;
        state.touchDragState.currentY = currentY;

        const x = currentX - state.touchDragState.offsetX;
        const y = currentY - state.touchDragState.offsetY;
        ...
        // Handle visual cue for "drag up" shortcut
        const DRAG_UP_TARGET_Y = 150;
        const header = document.querySelector('header');
        if (header && state.touchDragState.draggedCard?.dataset.status === 'waiting' && currentY < DRAG_UP_TARGET_Y) {
            header.classList.add('drop-target-active');
        } else if (header) {
            header.classList.remove('drop-target-active');
        }
 ...
 * Finalizes the drag-and-drop or cancels the long-press timer. Cleans up state.
 */
function handleTouchEnd() {
    // Cleanup visual cues
    document.querySelector('header')?.classList.remove('drop-target-active');
    ...
    const { isDragging, draggedCard, currentX, currentY } = state.touchDragState;
    ...
    if (isDragging && draggedCard) {
        ...
        // "Drag up to production" shortcut logic
        if (oldStatus === 'waiting' && orderId && currentY < DRAG_UP_TARGET_Y) {
            updateOrderStatus(orderId, 'inProduction');
        } else {
            // Original drop logic
            ...
            const dropZone = elementBelow?.closest<HTMLElement>('.kanban-column');
        ...
        // Cleanup styles regardless of drop success
    ...
    state.touchDragState = {
        isDragging: false, draggedCard: null, initialX: 0, initialY: 0,
 ...
 * This is a more reliable cross-browser alternative to copying to clipboard on long-press.
...
async function handleNotificationSubscription() {
    ...
    if (subscription) {
        ...
        try {
            // FIX: Removed check for placeholder VAPID key as it causes a type error now that the key is set.
